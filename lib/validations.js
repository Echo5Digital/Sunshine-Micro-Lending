...
export const applicationUpdateSchema = z
  ...
  .refine((data) => data.action !== 'status_change' || !!data.value, {
    message: 'A status value is required.',
    path: ['value'],
  })
  .refine(
    (data) => !(data.action === 'status_change' && data.value === 'declined') || !!data.reason?.trim(),
    { message: 'A reason is required when declining an application.', path: ['reason'] }
  );
